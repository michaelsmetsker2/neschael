# this file is a modified version of the importer script from ggsound, i dont wanna take credit for a lot of this
...
song_tracks = []
sfx_tracks = []
output_file = "../data/sound/tracks.s"
...
def generate_stream(track, order, channel, speed):
    ...
    while True:
        ...
        if instrument != current_instrument:
            note_output.append("STI, $%02X" % (instrument))
...
def main():
    ...
    if len(sys.argv) != 2:
        print("expects relative filepath")
        return
    ...
    with open(output_file, 'w') as f:
        ...
        # exports
        f.write(".INCLUDE \"lib/sound/sound.inc\"\n\n")
        f.write(".EXPORT song_list_low\n.EXPORT song_list_high\n")
        f.write(".EXPORT sfx_list_low\n.EXPORT sfx_list_high\n")
        f.write(".EXPORT instrument_list_low\n.EXPORT instrument_list_high\n\n")
        
            # song list
        f.write("song_list_low:\n")
        for track in song_tracks:
            f.write("\t.BYTE <%s\n" % (track["name"]))
        f.write("song_list_high:\n")
        for track in song_tracks:
            f.write("\t.BYTE >%s\n" % (track["name"]))
        f.write("\n")

        # sfx list
        f.write("sfx_list_low:\n")
        for track in sfx_tracks:
            f.write("\t.BYTE <%s\n" % (track["name"]))
        f.write("sfx_list_high:\n")
        for track in sfx_tracks:
            f.write("\t.BYTE >%s\n" % (track["name"]))
        f.write("\n")

        # instrument lists
        f.write("instrument_list_low:\n")
        for instrument in instruments:
            f.write("\t.BYTE <%s\n" % (instrument["name"]))
        f.write("instrument_list_high:\n")
        for instrument in instruments:
            f.write("\t.BYTE >%s\n" % (instrument["name"]))
        f.write("\n")
        ...
        for instrument in instruments:
            ...
            instrument_macros = [volume_macro, pitch_macro, duty_macro, arpeggio_macro]
            prefixes = ["", "", "DUTY_", ""]
            ...
            f.write("%s:\n" % instrument_name)
            f.write("\t.BYTE ")
            f.write("$%02X, " % instrument_offsets[0])
            f.write("$%02X, " % instrument_offsets[1])
            f.write("$%02X, " % instrument_offsets[2])
            f.write("$%02X, " % instrument_offsets[3])
            ...
            for channel in ["square1", "square2", "triangle", "noise"]:
                ...
                for order in unique_orders:
                    ...
                    if len(stream) > 0:
                        ...
                        channel_streams.append("\t.BYTE RET\n\n")
                if jump_frame == None:
                    jump_frame = 0
